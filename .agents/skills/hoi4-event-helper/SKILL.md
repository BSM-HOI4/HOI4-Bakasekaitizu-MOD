...
version: 1.0.0
description: Implement and edit concrete Hearts of Iron IV event blocks - country_event/news_event/state_event syntax, options, triggers, mean_time_to_happen, is_triggered_only, ai_chance, add_namespace. Use when writing or fixing actual event code in events/ files. For DESIGNING multi-event scenario chains or storylines use the hoi4-event skill first; for FINDING existing events use hoi4-searcher.
