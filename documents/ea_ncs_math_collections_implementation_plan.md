...
- [x] Tab5に同盟内・世界・7地域・危機・投資候補の表示モードを実装
- [x] 危機・投資必要度・準備金支援・通貨同盟の表示列を実装
- [x] 世界・地域・危機の抽出を名前付きCollectionsへ置換し、投資候補をEA内必要度降順で抽出
- [x] 追加GUI文言の日本語・英語ローカライズを実装
- [ ] HOI4実機で `reload interface`、全モードの対象、列表示、重複なしを確認
